...
class RadarGraphSlamNodelet : public nodelet::Nodelet, public ParamServer {
 public:
  ...
  virtual void onInit() {
    ...
    map_cloud_resolution =
        private_nh.param<double>("map_cloud_resolution", 0.05);
    ...
    max_keyframes_per_update =
        private_nh.param<int>("max_keyframes_per_update", 10);

    anchor_node = nullptr;
    anchor_edge = nullptr;
    floor_plane_node = nullptr;
    graph_slam.reset(new GraphSLAM(
        private_nh.param<std::string>("g2o_solver_type", "lm_var")));
    ...
    gps_edge_stddev_xy =
        private_nh.param<double>("gps_edge_stddev_xy", 10000.0);
    gps_edge_stddev_z = private_nh.param<double>("gps_edge_stddev_z", 10.0);
    max_gps_edge_stddev_xy =
        private_nh.param<double>("max_gps_edge_stddev_xy", 1.0);
    max_gps_edge_stddev_z =
        private_nh.param<double>("max_gps_edge_stddev_z", 2.0);

    // Preintegration Parameters
    enable_preintegration =
        private_nh.param<bool>("enable_preintegration", false);
    use_egovel_preinteg_trans =
        private_nh.param<bool>("use_egovel_preinteg_trans", false);
    preinteg_trans_stddev =
        private_nh.param<double>("preinteg_trans_stddev", 1.0);
    preinteg_orient_stddev =
        private_nh.param<double>("preinteg_orient_stddev", 2.0);
    ...
    barometer_edge_stddev =
        private_nh.param<double>("barometer_edge_stddev", 0.5);

    points_topic =
        private_nh.param<std::string>("points_topic", "/radar_enhanced_pcl");
    ...
    odom_sub.reset(new message_filters::Subscriber<nav_msgs::Odometry>(
        mt_nh, odomTopic, 256));
    cloud_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(
        mt_nh, "/filtered_points", 32));
    sync.reset(new message_filters::Synchronizer<ApproxSyncPolicy>(
        ApproxSyncPolicy(32), *odom_sub, *cloud_sub));
    sync->registerCallback(
        boost::bind(&RadarGraphSlamNodelet::cloud_callback, this, _1, _2));

    if (private_nh.param<bool>("enable_gps", true)) {
      gps_sub = mt_nh.subscribe("/gps/geopoint", 1024,
                                &RadarGraphSlamNodelet::gps_callback, this);
      nmea_sub = mt_nh.subscribe("/gpsimu_driver/nmea_sentence", 1024,
                                 &RadarGraphSlamNodelet::nmea_callback, this);
      navsat_sub = mt_nh.subscribe(
          gpsTopic, 1024, &RadarGraphSlamNodelet::navsat_callback, this);
    }
    if (private_nh.param<bool>("enable_barometer", false)) {
      barometer_sub =
          mt_nh.subscribe("/barometer/filtered", 16,
                          &RadarGraphSlamNodelet::barometer_callback, this);
    }
    if (enable_preintegration)
      imu_odom_sub =
          nh.subscribe("/imu_pre_integ/imu_odom_incre", 1024,
                       &RadarGraphSlamNodelet::imu_odom_callback, this);
    imu_sub =
        nh.subscribe("/imu", 1024, &RadarGraphSlamNodelet::imu_callback, this);
    command_sub = nh.subscribe("/command", 10,
                               &RadarGraphSlamNodelet::command_callback, this);

    //***** publishers ******
    markers_pub = mt_nh.advertise<visualization_msgs::MarkerArray>(
        "/radar_graph_slam/markers", 16);
    // Transform RadarOdom_to_base
    odom2base_pub = mt_nh.advertise<geometry_msgs::TransformStamped>(
        "/radar_graph_slam/odom2base", 16);
    aftmapped_odom_pub = mt_nh.advertise<nav_msgs::Odometry>(
        "/radar_graph_slam/aftmapped_odom", 16);
    aftmapped_odom_incremenral_pub = mt_nh.advertise<nav_msgs::Odometry>(
        "/radar_graph_slam/aftmapped_odom_incremental", 16);
    map_points_pub = mt_nh.advertise<sensor_msgs::PointCloud2>(
        "/radar_graph_slam/map_points", 1, true);
    read_until_pub =
        mt_nh.advertise<std_msgs::Header>("/radar_graph_slam/read_until", 32);
    odom_frame2frame_pub = mt_nh.advertise<nav_msgs::Odometry>(
        "/radar_graph_slam/odom_frame2frame", 16);

    dump_service_server = mt_nh.advertiseService(
        "/radar_graph_slam/dump", &RadarGraphSlamNodelet::dump_service, this);
    save_map_service_server =
        mt_nh.advertiseService("/radar_graph_slam/save_map",
                               &RadarGraphSlamNodelet::save_map_service, this);

    graph_updated = false;
    double graph_update_interval =
        private_nh.param<double>("graph_update_interval", 3.0);
    double map_cloud_update_interval =
        private_nh.param<double>("map_cloud_update_interval", 10.0);
    optimization_timer = mt_nh.createWallTimer(
        ros::WallDuration(graph_update_interval),
        &RadarGraphSlamNodelet::optimization_timer_callback, this);
    map_publish_timer = mt_nh.createWallTimer(
        ros::WallDuration(map_cloud_update_interval),
        &RadarGraphSlamNodelet::map_points_publish_timer_callback, this);

    if (dataset_name == "loop3")
      utm_to_world << -0.057621, 0.996222, -0.064972, -128453.624105, -0.998281,
          -0.058194, -0.006954, 361869.958099, -0.010708, 0.064459, 0.997863,
          -5882.237973, 0.000000, 0.000000, 0.000000, 1.000000;
    else if (dataset_name == "loop2")
      utm_to_world << -0.085585, 0.995774, -0.033303, -117561.214476, -0.996323,
          -0.085401, 0.006904, 364927.287181, 0.004031, 0.033772, 0.999421,
          -6478.377842, 0.000000, 0.000000, 0.000000, 1.000000;
  }
 ...
 private:
  ...
  void cloud_callback(const nav_msgs::OdometryConstPtr& odom_msg,
    ...
    if (keyframes.size() > 0) {
      ...
      Eigen::Isometry3d lastkeyframe_odom_incre =
          keyframe_last->odom_scan2scan.inverse() * odom_now;
      Eigen::Isometry3d keyframe_map2base_matrix =
          keyframe_last->node->estimate();
      // map2base = odom^(-1) * base
      matrix_map2base =
          (keyframe_map2base_matrix * lastkeyframe_odom_incre).matrix();
    }
    geometry_msgs::TransformStamped map2base_trans = matrix2transform(
        cloud_msg->header.stamp, matrix_map2base, mapFrame, baselinkFrame);
    if (pow(map2base_trans.transform.rotation.w, 2) +
            pow(map2base_trans.transform.rotation.x, 2) +
            pow(map2base_trans.transform.rotation.y, 2) +
            pow(map2base_trans.transform.rotation.z, 2) <
        pow(0.9, 2)) {
    ...
    // Get time of this key frame for Intergeration, to integerate between two
    // key frames
    ...
    KeyFrame::Ptr keyframe(
        new KeyFrame(keyframe_index, stamp, odom_now, accum_d, cloud));
    keyframe_index++;
    ...
    // - SINGLE_SCAN_FULL: using downsampled original point cloud
    // (/full_cloud_projected + downsampling)
    // - SINGLE_SCAN_FEAT: using surface feature as an input point cloud for
    // scan context (2020.04.01: checked it works.)
    // - MULTI_SCAN_FEAT: using NearKeyframes (because a MulRan scan does not
    // have beyond region, so to solve this issue ... )
    ...
    //     pcl::PointCloud<PointT>::Ptr multiKeyFrameFeatureCloud(new
    //     pcl::PointCloud<PointT>());
    //     loopFindNearKeyframes(multiKeyFrameFeatureCloud,
    //     cloudKeyPoses6D->size() - 1, historyKeyframeSearchNum);
  ...
  void imu_callback(const sensor_msgs::ImuConstPtr& imu_odom_msg) {
    ...
    if (cnt == 0) {
      ...
      tf::Quaternion orientation =
          tf::Quaternion(imu_quat_deskew.x(), imu_quat_deskew.y(),
                         imu_quat_deskew.z(), imu_quat_deskew.w());
      ...
      // Eigen::Vector3d eulerAngle = imu_mat_deskew.eulerAngles(0,1,2); // roll
      // pitch yaw
      ...
      std::cout << initial_pose(0, 0) << ", " << initial_pose(0, 1) << ", "
                << initial_pose(0, 2) << ", " << initial_pose(0, 3) << ", "
                << std::endl
                << initial_pose(1, 0) << ", " << initial_pose(1, 1) << ", "
                << initial_pose(1, 2) << ", " << initial_pose(1, 3) << ", "
                << std::endl
                << initial_pose(2, 0) << ", " << initial_pose(2, 1) << ", "
                << initial_pose(2, 2) << ", " << initial_pose(2, 3) << ", "
                << std::endl
                << initial_pose(3, 0) << ", " << initial_pose(3, 1) << ", "
                << initial_pose(3, 2) << ", " << initial_pose(3, 3) << ", "
                << std::endl
                << std::endl;
      cnt = 1;
    }
  ...
  geometry_msgs::Transform preIntegrationTransform() {
    ...
    while (!imu_odom_queue.empty() &&
           imu_odom_queue.front()->header.stamp.toSec() <
               lastKeyframeTime - delta_t) {
      lastImuOdomQT = imu_odom_queue.front()->header.stamp.toSec();
      imu_odom_queue.pop_front();
    }
    // pop old Twist message
    while (!twist_queue.empty() && twist_queue.front()->header.stamp.toSec() <
                                       lastKeyframeTime - delta_t) {
      // lastTwistQT = twist_queue.front()->header.stamp.toSec();
      twist_queue.pop_front();
    }
    // repropogate
    Eigen::Isometry3d isom_frame2frame;   // Translation increment between last
                                          // key frame and this IMU msg
    ...
    geometry_msgs::Vector3
        translation_frame2frame;  // Translation between two key frames
    if (!imu_odom_queue.empty()) {
      // (Doing this because some imu_odom later than thisKeyframeTime will be
      // recieved)
      ...
      if (use_egovel_preinteg_trans) {
        ...
        Eigen::MatrixXd q_via_x(twist_length, 3), q_via_y(twist_length, 3),
            q_via_z(twist_length, 3);
        Eigen::VectorXd t_via_x(twist_length), t_via_y(twist_length),
            t_via_z(twist_length);
        // double dt = 1.0/12;
        for (size_t i = 0; i < twist_length; i++) {
          q_via_x(i, 0) = twist_queue.at(i)->twist.linear.x;
          q_via_x(i, 1) = 0;
          q_via_x(i, 2) = 0;
          // acc: q(i,1)=(twist_queue.at(i+1).twist.linear.x -
          // twist_queue.at(i).twist.linear.x) / dt;
        ...
        egovelIntegratorX.reset(new CubicInterpolation(
            q_via_x, t_via_x));  // Integrator of ego velocity at X axis
        egovelIntegratorY.reset(new CubicInterpolation(
            q_via_y, t_via_y));  // Integrator of ego velocity at Y axis
        egovelIntegratorZ.reset(new CubicInterpolation(
            q_via_z, t_via_z));  // Integrator of ego velocity at Z axis
        // ROS_INFO("Twist queue - Start time: %f Stop time: %f", t_via_x(0),
        // t_via_x(t_via_x.size()-1));
      }
      Eigen::Isometry3d isom_imu_odom_last(Eigen::Isometry3d::Identity());
      Eigen::Quaterniond q_imu_odom(
          imu_odom_queue.front()->pose.pose.orientation.w,
      ...
      isom_imu_odom_last.pretranslate(
          Eigen::Vector3d(imu_odom_queue.front()->pose.pose.position.x,
      ...
      q_imu_odom = Eigen::Quaterniond(
          imu_odom_queue.at(imu_odom_end_index)->pose.pose.orientation.w,
      ...
      isom_imu_odom_this.pretranslate(Eigen::Vector3d(
          imu_odom_queue.at(imu_odom_end_index)->pose.pose.position.x,
          imu_odom_queue.at(imu_odom_end_index)->pose.pose.position.y,
          imu_odom_queue.at(imu_odom_end_index)->pose.pose.position.z));
      Eigen::Isometry3d isom_imu_odom_btn =
          isom_imu_odom_last.inverse() * isom_imu_odom_this;
      // cout << isom_imu_odom_btn.matrix() << endl;
      // ROS_INFO("IMU Odom queue - Start time: %f Stop time: %f",
      // imu_odom_queue.at(0)->header.stamp.toSec(),
      // imu_odom_queue.at(imu_odom_end_index)->header.stamp.toSec());
      if (use_egovel_preinteg_trans) {  // Use Ego vel as translation
                                        // integration
        for (size_t i = 0; i < imu_odom_end_index + 1; i++) {
          ...
          double dt = (lastImuOdomQT < 0) ? (1.0 / 200.0)
                                          : (imuOdomTime - lastImuOdomQT);
          Eigen::Vector3d translation_ =
              Eigen::Vector3d(egovelIntegratorX->getPosition(imuOdomTime),
  ...
  bool flush_barometer_queue() {
    ...
    for (size_t i = 0; i < keyframes.size(); i++) {
      ...
      if (keyframe->stamp < (*barometer_cursor)->header.stamp ||
          keyframe->altitude) {
        continue;
      }
      // find the barometer data which is closest to the keyframe_
      auto closest_barometer = barometer_cursor;
      for (auto baro = barometer_cursor; baro != barometer_queue.end();
           baro++) {
        auto dt =
            ((*closest_barometer)->header.stamp - keyframe->stamp).toSec();
      ...
      // if the time residual between the barometer and keyframe_ is too large,
      // skip it
      ...
      Eigen::Vector4d aftmapped_pos(keyframe->odom_scan2scan.translation().x(),
                                    keyframe->odom_scan2scan.translation().y(),
                                    (*closest_barometer)->altitude, 1.0);
      aftmapped_pos = initial_pose * aftmapped_pos;
      // std::cout << "baro position: " << aftmapped_pos(0) << ", " <<
      // aftmapped_pos(1) << ", " << aftmapped_pos(2) << std::endl;

      nav_msgs::Odometry initial_odom;  // Initial posture
      initial_odom =
          matrix2odom(keyframe->stamp, initial_pose, mapFrame, baselinkFrame);
      ...
      if (enable_barometer) {
        ...
        if (barometer_edge_type == 1) {
          ...
          Eigen::Vector1d information_matrix =
              Eigen::Vector1d::Identity() / barometer_edge_stddev;
          edge = graph_slam->add_se3_prior_z_edge(keyframe->node, z,
                                                  information_matrix);
          graph_slam->add_robust_kernel(
              edge,
              private_nh.param<std::string>("barometer_edge_robust_kernel",
                                            "NONE"),
              private_nh.param<double>("barometer_edge_robust_kernel_size",
                                       1.0));
        } else if (barometer_edge_type == 2 && i != 0 &&
                   keyframes.at(i - 1)->altitude.is_initialized()) {
          g2o::OptimizableGraph::Edge* edge;
          Eigen::Vector1d information_matrix =
              Eigen::Vector1d::Identity() / barometer_edge_stddev;
          Eigen::Vector1d relative_z(keyframe->altitude.value() -
                                     keyframes.at(i - 1)->altitude.value());
          edge = graph_slam->add_se3_z_edge(keyframe->node,
                                            keyframes.at(i - 1)->node,
                                            relative_z, information_matrix);
          graph_slam->add_robust_kernel(
              edge,
              private_nh.param<std::string>("barometer_edge_robust_kernel",
                                            "NONE"),
    ...
    auto remove_loc = std::upper_bound(
        barometer_queue.begin(), barometer_queue.end(), keyframes.back()->stamp,
        [=](const ros::Time& stamp,
            const barometer_bmp388::BarometerConstPtr& baropoint) {
  ...
  /**
   * @brief this method adds all the keyframes_ in #keyframe_queue to the pose
   * graph (odometry edges)
    ...
    for (int i = 0;
      ...
      if (keyframes.empty() && new_keyframes.size() == 1) {
        if (private_nh.param<bool>("fix_first_node", false)) {
          ...
          std::stringstream sst(private_nh.param<std::string>(
      ...
      // relative pose between odom of previous frame and this frame R2=R12*R1
      // => R12 = inv(R2) * R1
      ...
      auto edge = graph_slam->add_se3_edge(keyframe->node, prev_keyframe->node,
      ...
      if (enable_preintegration) {
        ...
        g2o::SE3Quat relative_se3quat(
            Eigen::Quaterniond(
                relative_trans.rotation.w, relative_trans.rotation.x,
                relative_trans.rotation.y, relative_trans.rotation.z),
            Eigen::Vector3d(relative_trans.translation.x,
                            relative_trans.translation.y,
                            relative_trans.translation.z));
        Eigen::Isometry3d relative_isometry =
            transform2isometry(relative_trans);
        Eigen::MatrixXd information_integ = Eigen::MatrixXd::Identity(6, 6);
        information_integ << 1.0 / preinteg_trans_stddev, 0, 0, 0, 0, 0, 0,
            1.0 / preinteg_trans_stddev, 0, 0, 0, 0, 0, 0,
            1.0 / preinteg_trans_stddev, 0, 0, 0, 0, 0, 0,
            1.0 / preinteg_orient_stddev, 0, 0, 0, 0, 0, 0,
            1.0 / preinteg_orient_stddev, 0, 0, 0, 0, 0, 0,
            1.0 / preinteg_orient_stddev;
        auto edge_integ =
            graph_slam->add_se3_edge(prev_keyframe->node, keyframe->node,
                                     relative_isometry, information_integ);
        graph_slam->add_robust_kernel(
            edge_integ,
            private_nh.param<std::string>("integ_edge_robust_kernel", "NONE"),
            private_nh.param<double>("integ_edge_robust_kernel_size", 1.0));
      }
    }

    std_msgs::Header read_until;
    read_until.stamp =
        keyframe_queue[num_processed]->stamp + ros::Duration(10, 0);
    read_until.frame_id = points_topic;
    read_until_pub.publish(read_until);
    read_until.frame_id = "/filtered_points";
    read_until_pub.publish(read_until);
  ...
  /**
   * @brief Back-end Optimization. This methods adds all the data in the queues
   * to the pose graph, and then optimizes the pose graph
   * @param event
   */
  void optimization_timer_callback(const ros::WallTimerEvent& event) {
    ...
    // Copy "new_keyframes_" to vector  "keyframes_", "new_keyframes_" was used
    // for loop detaction
    std::copy(new_keyframes.begin(), new_keyframes.end(),
              std::back_inserter(keyframes));
    new_keyframes.clear();

    if (private_nh.param<bool>("enable_loop_closure", false)) addLoopFactor();

    // move the first node / position to the current estimate of the first node
    // pose so the first node moves freely while trying to stay around the
    // origin
    if (anchor_node &&
        private_nh.param<bool>("fix_first_node_adaptive", true)) {
    ...
    int num_iterations =
        private_nh.param<int>("g2o_solver_num_iterations", 1024);
    ...
    nav_msgs::Odometry aft = isometry2odom(keyframe->stamp, trans_aftmapped,
                                           mapFrame, odometryFrame);
    aftmapped_odom_pub.publish(aft);

    // Publish After-Mapped Odometry Incrementation
    nav_msgs::Odometry aft_incre = isometry2odom(
        keyframe->stamp, trans_aftmapped_incremental, mapFrame, odometryFrame);
    aftmapped_odom_incremenral_pub.publish(aft_incre);

    // Publish /odom to /base_link
    if (odom2base_pub.getNumSubscribers()) {  // Returns the number of
                                              // subscribers that are currently
                                              // connected to this Publisher
      geometry_msgs::TransformStamped ts = matrix2transform(
          keyframe->stamp, trans.matrix(), mapFrame, odometryFrame);
  ...
  void addLoopFactor() {
    if (loop_detector->loopIndexQueue.empty()) return;
    for (int i = 0; i < (int)loop_detector->loopIndexQueue.size(); ++i) {
      ...
      auto edge = graph_slam->add_se3_edge(keyframes[indexFrom]->node,
                                           keyframes[indexTo]->node,
                                           poseBetween, information_matrix);
      graph_slam->add_robust_kernel(
          edge,
          private_nh.param<std::string>("loop_closure_edge_robust_kernel",
                                        "NONE"),
          private_nh.param<double>("loop_closure_edge_robust_kernel_size",
                                   1.0));
    }
   ...
   * @return
   */
  visualization_msgs::MarkerArray create_marker_array(
    ...
    for (auto it = loop_detector->loopIndexContainer.begin();
         it != loop_detector->loopIndexContainer.end(); ++it) {
    ...
    for (int i = 0; edge_itr != graph_slam->graph->edges().end();
         edge_itr++, i++) {
      ...
      if (edge_se3) {
        ...
        double p1 = static_cast<double>(v1->id()) /
                    graph_slam->graph->vertices().size();
        double p2 = static_cast<double>(v2->id()) /
                    graph_slam->graph->vertices().size();
      ...
      if (edge_plane) {
        g2o::VertexSE3* v1 =
            dynamic_cast<g2o::VertexSE3*>(edge_plane->vertices()[0]);
        Eigen::Vector3d pt1 = v1->estimate().translation();
      ...
      g2o::EdgeSE3PriorXY* edge_priori_xy =
          dynamic_cast<g2o::EdgeSE3PriorXY*>(edge);
      if (edge_priori_xy) {
        g2o::VertexSE3* v1 =
            dynamic_cast<g2o::VertexSE3*>(edge_priori_xy->vertices()[0]);
        Eigen::Vector3d pt1 = v1->estimate().translation();
      ...
      g2o::EdgeSE3PriorXYZ* edge_priori_xyz =
          dynamic_cast<g2o::EdgeSE3PriorXYZ*>(edge);
      if (edge_priori_xyz) {
        g2o::VertexSE3* v1 =
            dynamic_cast<g2o::VertexSE3*>(edge_priori_xyz->vertices()[0]);
        Eigen::Vector3d pt1 = v1->estimate().translation();
   ...
   * @return
   */
  bool dump_service(radar_graph_slam::DumpGraphRequest& req,
                    radar_graph_slam::DumpGraphResponse& res) {
    std::lock_guard<std::mutex> lock(main_thread_mutex);
    ...
    if (zero_utm) {
      ...
      zero_utm_ofs << boost::format("%.6f %.6f %.6f") % zero_utm->x() %
                          zero_utm->y() % zero_utm->z()
                   << std::endl;
    }
    ...
    ofs << "anchor_edge " << (anchor_edge == nullptr ? -1 : anchor_edge->id())
        << std::endl;
    ofs << "floor_node "
        << (floor_plane_node == nullptr ? -1 : floor_plane_node->id())
        << std::endl;
   ...
   * @return
   */
  bool save_map_service(radar_graph_slam::SaveMapRequest& req,
                        radar_graph_slam::SaveMapResponse& res) {
    std::vector<KeyFrameSnapshot::Ptr> snapshot;
  ...
  void nmea_callback(const nmea_msgs::SentenceConstPtr& nmea_msg) {
    ...
    if (grmc.status != 'A') return;
    geographic_msgs::GeoPointStampedPtr gps_msg(
        new geographic_msgs::GeoPointStamped());
  ...
  bool flush_gps_queue() {
    ...
    for (auto& keyframe : keyframes) {
      ...
      // if the time residual between the gps and keyframe_ is too large, skip
      // it
      gps_cursor = closest_gps;
      if (0.2 <
          std::abs(((*closest_gps).header.stamp - keyframe->stamp).toSec())) {
        continue;
      }

      // convert (latitude, longitude, altitude) -> (easting, northing,
      // altitude) in UTM coordinate
      ...
      if (cov_x > max_gps_edge_stddev_xy || cov_y > max_gps_edge_stddev_xy ||
          cov_z > max_gps_edge_stddev_z)
        continue;
      ...
      Eigen::Vector3d world_coordinate =
          (utm_to_world *
           Eigen::Vector4d(utm.easting, utm.northing, utm.altitude, 1))
              .head(3);
      Eigen::Vector3d trans_err =
          keyframe->node->estimate().translation() - world_coordinate;
      if (trans_err.norm() < 5.0) continue;
      //********** G2O Edge ***********
      g2o::OptimizableGraph::Edge* edge;
      if (std::isnan(world_coordinate.z())) {
        Eigen::Matrix2d information_matrix =
            Eigen::Matrix2d::Identity() / cov_x;
        edge = graph_slam->add_se3_prior_xy_edge(
            keyframe->node, world_coordinate.head<2>(), information_matrix);
      } else {
        Eigen::Matrix3d information_matrix = Eigen::Matrix3d::Identity();
        information_matrix(0, 0) /= cov_x;
        information_matrix(1, 1) /= cov_y;
        information_matrix(2, 2) /= cov_z;
        edge = graph_slam->add_se3_prior_xyz_edge(
            keyframe->node, world_coordinate, information_matrix);
      }
      graph_slam->add_robust_kernel(
          edge, private_nh.param<std::string>("gps_edge_robust_kernel", "NONE"),
    ...
    auto remove_loc = std::upper_bound(
        gps_navsat_queue.begin(), gps_navsat_queue.end(),
        keyframes.back()->stamp,
        [=](const ros::Time& stamp, const sensor_msgs::NavSatFix& geopoint) {
  ...
  void command_callback(const std_msgs::String& str_msg) {
    if (str_msg.data == "output_aftmapped") {
      ...
      if (loop_detector->pf_time.size() > 0) {
        ...
        double median = loop_detector->pf_time.at(
            floor((double)loop_detector->pf_time.size() / 2));
      ...
      if (loop_detector->sc_time.size() > 0) {
        ...
        double median = loop_detector->sc_time.at(
            floor((double)loop_detector->sc_time.size() / 2));
      ...
      if (loop_detector->oc_time.size() > 0) {
        ...
        double median = loop_detector->oc_time.at(
            floor((double)loop_detector->oc_time.size() / 2));
 ...
 private:
  ...
  Eigen::Matrix4d
      trans_odom2map;  // keyframe->node->estimate() * keyframe->odom.inverse();
...
PLUGINLIB_EXPORT_CLASS(radar_graph_slam::RadarGraphSlamNodelet,
                       nodelet::Nodelet)
