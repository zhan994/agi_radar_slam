...
class ScanMatchingOdometryNodelet : public nodelet::Nodelet,
 ...
 public:
  ...
  // geometry_msgs::TransformStamped> ApproxSyncPolicy2;
  ...
  /**
   * \brief // api: 前端里程计ScanMatchingOdometryNodelet主函数
   * 
   */
  virtual void onInit() {
    ...
    // step: 1 获取句柄
    nh = getNodeHandle();
    mt_nh = getMTNodeHandle(); // 多线程
    private_nh = getPrivateNodeHandle();

    // step: 2 初始化参数
    initialize_params();  // this

    // note: 不用这些数据，enable_imu_frontend=false
    if (private_nh.param<bool>("enable_imu_frontend", false)) {
      msf_pose_sub = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>(
          "/msf_core/pose", 1,
          boost::bind(&ScanMatchingOdometryNodelet::msf_pose_callback, this, _1,
                      false));
      msf_pose_after_update_sub =
          nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>(
              "/msf_core/pose_after_update", 1,
              boost::bind(&ScanMatchingOdometryNodelet::msf_pose_callback, this,
                          _1, true));
    }

    // step: 3 订阅雷达估计的速度、过滤后的点云
    //******** Subscribers **********
    ego_vel_sub.reset(new message_filters::Subscriber<
                      geometry_msgs::TwistWithCovarianceStamped>(
        mt_nh, "/eagle_data/twist", 256));
    points_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(
        mt_nh, "/filtered_points", 32));
    sync.reset(new message_filters::Synchronizer<ApproxSyncPolicy>(
        ApproxSyncPolicy(32), *ego_vel_sub, *points_sub));
    sync->registerCallback(boost::bind(
        &ScanMatchingOdometryNodelet::pointcloud_callback, this, _1, _2));
    imu_sub = nh.subscribe("/imu", 1024,
                           &ScanMatchingOdometryNodelet::imu_callback, this);
    command_sub = nh.subscribe(
        "/command", 10, &ScanMatchingOdometryNodelet::command_callback, this);

    //******** Publishers **********
    read_until_pub = nh.advertise<std_msgs::Header>(
    ...
    trans_pub = nh.advertise<geometry_msgs::TransformStamped>(
        "/scan_matching_odometry/transform", 32);
    status_pub = private_nh.advertise<ScanMatchingStatus>(
        "/scan_matching_odometry/status", 8);
    aligned_points_pub =
        nh.advertise<sensor_msgs::PointCloud2>("/aligned_points", 32);
    submap_pub =
        nh.advertise<sensor_msgs::PointCloud2>("/radar_graph_slam/submap", 2);
  }
 ...
 private:
  ...
  void initialize_params() {
    auto& pnh = private_nh;
    points_topic =
        pnh.param<std::string>("points_topic", "/radar_enhanced_pcl");
    ...
    enable_transform_thresholding =
        pnh.param<bool>("enable_transform_thresholding", false);
    ...
    // graph_slam.reset(new GraphSLAM(pnh.param<std::string>("g2o_solver_type",
    // "lm_var")));

    // select a downsample method (VOXELGRID, APPROX_VOXELGRID, NONE)
    std::string downsample_method =
        pnh.param<std::string>("downsample_method", "VOXELGRID");
    double downsample_resolution =
        pnh.param<double>("downsample_resolution", 0.1);
    if (downsample_method == "VOXELGRID") {
      std::cout << "downsample: VOXELGRID " << downsample_resolution
                << std::endl;
  ...
  void imu_callback(const sensor_msgs::ImuConstPtr& imu_msg) {
    ...
    if (cnt == 0) {
      geometry_msgs::Quaternion imuQuat =
          tf::createQuaternionMsgFromRollPitchYaw(roll, pitch, 0);
      global_orient_matrix =
          Eigen::Quaterniond(imuQuat.w, imuQuat.x, imuQuat.y, imuQuat.z)
              .toRotationMatrix();
  ...
  bool flush_imu_queue() {
    ...
    for (size_t i = 0; i < keyframes.size(); i++) {
      ...
      // if the time residual between the imu and keyframe_ is too large, skip
      // it
      imu_cursor = closest_imu;
      if (0.2 <
          std::abs(((*closest_imu)->header.stamp - keyframe->stamp).toSec())) {
        continue;
      }
      ...
      imu_.angular_velocity_covariance =
          (*closest_imu)->angular_velocity_covariance;
      imu_.linear_acceleration_covariance =
          (*closest_imu)->linear_acceleration_covariance;
      imu_.orientation_covariance = (*closest_imu)->orientation_covariance;
      keyframe->imu = imu_;
      updated = true;
    }
    auto remove_loc = std::upper_bound(
        imu_queue.begin(), imu_queue.end(), keyframes.back()->stamp,
        [=](const ros::Time& stamp, const sensor_msgs::ImuConstPtr& imupoint) {
  ...
  std::pair<bool, sensor_msgs::Imu> get_closest_imu(ros::Time frame_stamp) {
    ...
    if (keyframes.empty() || imu_queue.empty()) return false_result;
    bool updated = false;
    auto imu_cursor = imu_queue.begin();
  ...
  void transformUpdate(Eigen::Matrix4d& odom_to_update)  // IMU
  {
    if (imuPointerLast >= 0) {
      ...
      geometry_msgs::Quaternion imuQuat =
          tf::createQuaternionMsgFromRollPitchYaw(imuRollLast, imuPitchLast,
                                                  ypr_odom(0));
      Eigen::Matrix3d imu_rot = Eigen::Matrix3d(
          Eigen::Quaterniond(imuQuat.w, imuQuat.x, imuQuat.y, imuQuat.z));
      Eigen::Vector3d ypr_imu = R2ypr(imu_rot);
      // IMU orientation transformed from world coordinate to map coordinate
      Eigen::Matrix3d imu_rot_transed =
          global_orient_matrix.inverse() * imu_rot;
      Eigen::Vector3d ypr_imu_trans = R2ypr(imu_rot_transed);
      double& yaw_ = ypr_odom(0);
      double pitch_fused = (1 - imu_fusion_ratio) * ypr_odom(1) +
                           imu_fusion_ratio * ypr_imu_trans(1);
      double roll_fused = (1 - imu_fusion_ratio) * ypr_odom(2) +
                          imu_fusion_ratio * ypr_imu_trans(2);
      geometry_msgs::Quaternion rosQuat =
          tf::createQuaternionMsgFromRollPitchYaw(roll_fused, pitch_fused,
                                                  yaw_);
      Eigen::Quaterniond quat_updated =
          Eigen::Quaterniond(rosQuat.w, rosQuat.x, rosQuat.y, rosQuat.z);
      odom_to_update.block<3, 3>(0, 0) = quat_updated.toRotationMatrix();

      if (imu_debug_out)
        cout << "IMU rp: " << RAD2DEG(ypr_imu(2)) << " " << RAD2DEG(ypr_imu(1))
             << ". IMU transed rp: " << RAD2DEG(ypr_imu_trans(2)) << " "
             << RAD2DEG(ypr_imu_trans(1))
             // << ". Odom rp: " << RAD2DEG(ypr_odom(2)) << " " <<
             // RAD2DEG(ypr_odom(1))
             // << ". Updated rp: " << RAD2DEG(roll_fused) << " " <<
             // RAD2DEG(pitch_fused)
             // << ". Roll Pitch increment: " << RAD2DEG(roll_fused -
             // ypr_odom(2)) << " " << RAD2DEG(pitch_fused - ypr_odom(1))
             << endl;
    }
  }
  ...
  void pointcloud_callback(
      const geometry_msgs::TwistWithCovarianceStampedConstPtr& twistMsg,
    ...
    // If too large, set 0
    if (pow(egovel_cum_x, 2) + pow(egovel_cum_y, 2) + pow(egovel_cum_z, 2) >
        pow(max_egovel_cum, 2))
      ;
    else
      egovel_cum.block<3, 1>(0, 3) =
          Eigen::Vector3d(egovel_cum_x, egovel_cum_y, egovel_cum_z);
    ...
    publish_odometry(cloud_msg->header.stamp, mapFrame, odometryFrame, pose,
                     twist);

    // In offline estimation, point clouds will be supplied until the published
    // time
  ...
  /**
   * @brief estimate the relative pose between an input cloud and a keyframe_
   * cloud
   ...
   */
  Eigen::Matrix4d matching(const ros::Time& stamp,
    ...
    if (enable_scan_to_map) registration_s2m->setInputSource(filtered);
    ...
    publish_scan_matching_status(stamp, cloud->header.frame_id, aligned,
                                 msf_source, msf_delta);
    ...
    Eigen::Matrix4d trans_s2s =
        registration_s2s->getFinalTransformation().cast<double>();
    ...
    // Add abnormal judgment, that is, if the difference between the two frames
    // matching point cloud transition matrix is too large, it will be discarded
    ...
    if (enable_transform_thresholding) {
      ...
      // double da_rd = std::acos(Eigen::Quaterniond(radar_delta.block<3, 3>(0,
      // 0)).w())*180/M_PI;
      ...
      bool too_large_trans =
          dx_rd > max_acceptable_trans || da_rd > max_acceptable_angle;
      double da, dx, delta_rot_imu = 0;
      Eigen::Matrix3d matrix_rot;
      Eigen::Vector3d delta_trans_egovel;

      if (enable_imu_thresholding) {
        // Use IMU orientation to determine whether the matching result is good
        // or not
        ...
        if (result.first) {
          ...
          Eigen::Quaterniond imu_quat(
              frame_imu.orientation.w, frame_imu.orientation.x,
              frame_imu.orientation.y, frame_imu.orientation.z);
          Eigen::Quaterniond prev_imu_quat(
              last_frame_imu.orientation.w, last_frame_imu.orientation.x,
              last_frame_imu.orientation.y, last_frame_imu.orientation.z);
          rot_imu = (prev_imu_quat.inverse() * imu_quat).toRotationMatrix();
          Eigen::Vector3d eulerAngle_imu =
              rot_imu.eulerAngles(0, 1, 2);  // roll pitch yaw
          Eigen::Vector3d eulerAngle_rd =
              last_radar_delta.block<3, 3>(0, 0).eulerAngles(0, 1, 2);
          Eigen::AngleAxisd rollAngle(Eigen::AngleAxisd(
              restrict_rad(eulerAngle_imu(0)), Eigen::Vector3d::UnitX()));
          Eigen::AngleAxisd pitchAngle(Eigen::AngleAxisd(
              restrict_rad(eulerAngle_imu(1)), Eigen::Vector3d::UnitY()));
          Eigen::AngleAxisd yawAngle(Eigen::AngleAxisd(
              restrict_rad(eulerAngle_rd(2)), Eigen::Vector3d::UnitZ()));
          matrix_rot = yawAngle * pitchAngle * rollAngle;
          da = fabs(std::acos(
                   Eigen::Quaterniond(rot_rd.inverse() * rot_imu).w())) *
               180 / M_PI;
          delta_rot_imu =
              fabs(std::acos(Eigen::Quaterniond(rot_imu).w())) * 180 / M_PI;
          last_frame_imu = frame_imu;
        }
        delta_trans_egovel = egovel_cum.block<3, 1>(0, 3).cast<double>();
        Eigen::Vector3d delta_trans_radar =
            radar_delta.block<3, 1>(0, 3).cast<double>();
        ...
        if (dx > max_diff_trans || da > max_diff_angle || too_large_trans) {
          ...
          if (too_large_trans)
            cout << "Too large transform! " << dx_rd << "[m] " << da_rd
                 << "[deg] ";
          cout << "Difference of Odom and IMU/EgoVel too large " << dx << "[m] "
               << da << "[deg] (" << stamp << ")" << endl;
      ...
      if (0) {
        cout << "radar trans:" << dx_rd << " m rot:" << da_rd
             << " degree. EgoVel " << delta_trans_egovel.norm() << " m. "
    ...
    if (keyframe_updater->decide(Eigen::Isometry3d(odom_s2s_now), stamp)) {
      ...
      KeyFrame::Ptr keyframe(new KeyFrame(
          keyframe_index, stamp, Eigen::Isometry3d(odom_s2s_now.cast<double>()),
          accum_d, cloud));
      keyframe_index++;
      ...
      if (enable_scan_to_map) {
        pcl::PointCloud<PointT>::Ptr submap_cloud(
            new pcl::PointCloud<PointT>());
        pcl::PointCloud<PointT>::ConstPtr submap_cloud_downsampled;
        for (size_t i = std::max(0, (int)keyframes.size() - max_submap_frames);
             i < keyframes.size() - 1; i++) {
          Eigen::Matrix4d rel_pose =
              keyframes.at(i)->odom_scan2scan.matrix().inverse() *
              keyframes.back()->odom_scan2scan.matrix();
          pcl::PointCloud<PointT>::Ptr cloud_transformed(
              new pcl::PointCloud<PointT>());
          pcl::transformPointCloud(*keyframes.at(i)->cloud, *cloud_transformed,
                                   rel_pose);
  ...
  void publish_odometry(const ros::Time& stamp,
                        const std::string& father_frame_id,
                        const std::string& child_frame_id,
                        const Eigen::Matrix4d& pose_in,
                        const geometry_msgs::TwistWithCovariance twist_in) {
    // publish transform stamped for IMU integration
    geometry_msgs::TransformStamped odom_trans = matrix2transform(
        stamp, pose_in, father_frame_id, child_frame_id);  //"map"
  ...
  void publish_scan_matching_status(
      ...
      const std::string& msf_source, const Eigen::Isometry3d& msf_delta) {
    ...
    for (int i = 0; i < aligned->size(); i++) {
      ...
      registration_s2s->getSearchMethodTarget()->nearestKSearch(
    ...
    status.relative_pose = isometry2pose(Eigen::Isometry3d(
        registration_s2s->getFinalTransformation().cast<double>()));
  ...
  void command_callback(const std_msgs::String& str_msg) {
    if (str_msg.data == "time") {
      ...
      double median =
          s2s_matching_time.at(size_t(s2s_matching_time.size() / 2));
 ...
 private:
  ...
  bool imu_debug_out;
  Eigen::Matrix3d global_orient_matrix;  // The rotation matrix with initial IMU
                                         // roll & pitch measurement (yaw = 0)
  ...
  pcl::PointCloud<PointT>::ConstPtr
      keyframe_cloud_s2s;  // keyframe_ point cloud
  pcl::PointCloud<PointT>::ConstPtr
      keyframe_cloud_s2m;  // keyframe_ point cloud

  // Registration
  pcl::Filter<PointT>::Ptr downsample_filter;
  pcl::Registration<PointT, PointT>::Ptr
      registration_s2s;  // Scan-to-Scan Registration
  pcl::Registration<PointT, PointT>::Ptr
      registration_s2m;  // Scan-to-Submap Registration
...
PLUGINLIB_EXPORT_CLASS(radar_graph_slam::ScanMatchingOdometryNodelet,
                       nodelet::Nodelet)
