// This file is part of RIO - Radar Inertial Odometry and Radar ego velocity
// estimation. Copyright (C) 2021  Christopher Doer <christopher.doer@kit.edu>
...
/**
 * \brief // api: 转为radar点云数据
 * 
 * \param item 
 * \param idx 
 * \return RadarPointCloudType 
 */
static RadarPointCloudType toRadarPointCloudType(
    const Vector11& item, const RadarEgoVelocityEstimatorIndices& idx) {
...
bool RadarEgoVelocityEstimator::estimate(
    const sensor_msgs::PointCloud2& radar_scan_msg, Vector3& v_r,
    Vector3& sigma_v_r) {
...
/**
 * \brief // api: 毫米波ego_velocity速度估计
 *
 * \param radar_scan_msg 点云
 * \param v_r 估计输出的速度
 * \param sigma_v_r 估计输出的速度的方差
 * \param inlier_radar_msg 静态点
 * \param outlier_radar_msg 动态点
 * \return true
 * \return false
 */
bool RadarEgoVelocityEstimator::estimate(
    const sensor_msgs::PointCloud2& radar_scan_msg, Vector3& v_r,
    Vector3& sigma_v_r, sensor_msgs::PointCloud2& inlier_radar_msg,
    sensor_msgs::PointCloud2& outlier_radar_msg) {
  // step: 1 分配radar点云
  auto radar_scan(new pcl::PointCloud<RadarPointCloudType>);
  auto radar_scan_inlier(
      new pcl::PointCloud<RadarPointCloudType>);  // static objects
  auto radar_scan_outlier(new pcl::PointCloud<RadarPointCloudType>);
  bool success = false;
  pcl::fromROSMsg(radar_scan_msg, *radar_scan);

  // step: 2 ego_vel估计，静动态点判断
  if (1)  // pcl2msgToPcl(radar_scan_msg, *radar_scan)
  {
    std::vector<Vector11> valid_targets;
    // step: 2.1 遍历点云数据，获取距离、fov以及snr有效的数据
    for (uint i = 0; i < radar_scan->size(); ++i) {
      ...
      Real elevation =
          std::atan2(std::sqrt(target.x * target.x + target.y * target.y),
                     target.z) -
          M_PI_2;
      if (r > config_.min_dist && r < config_.max_dist &&
          target.intensity > config_.min_db &&
          std::fabs(azimuth) <
              angles::from_degrees(config_.azimuth_thresh_deg) &&
          std::fabs(elevation) <
              angles::from_degrees(config_.elevation_thresh_deg)) {
        Vector11 v_pt;  // features of a point
        v_pt << target.x, target.y, target.z, target.intensity,
            -target.doppler * config_.doppler_velocity_correction_factor, r,
            azimuth, elevation, target.x / r, target.y / r, target.z / r;
        valid_targets.emplace_back(v_pt);
      }
      // else{
      //   cout << (r > config_.min_dist) << (r < config_.max_dist) <<
      //   (target.intensity > config_.min_db) << \
      //   (std::fabs(azimuth) <
      //   angles::from_degrees(config_.azimuth_thresh_deg)) <<
      //   (std::fabs(elevation) <
      //   angles::from_degrees(config_.elevation_thresh_deg)) << \ " " <<
      //   angles::to_degrees(azimuth) << " " << angles::to_degrees(elevation)
      //   << endl;
      // }
    }
    ...
    if (valid_targets.size() > 2) {
      ...
      // step: 2.2 外点比例位置的第n小速度对比零速阈值并过滤所有点云
      const size_t n =
          v_dopplers.size() * (1.0 - config_.allowed_outlier_percentage);
      std::nth_element(v_dopplers.begin(), v_dopplers.begin() + n,
                       v_dopplers.end());
      ...
      if (median < config_.thresh_zero_velocity) {
        ...
        v_r = Vector3(0, 0, 0);
        sigma_v_r = Vector3(config_.sigma_zero_velocity_x,
        ...
        // step: 2.3 LSQ velocity estimation
        // step: 2.3.1 获取所有观测数据
        Matrix radar_data(valid_targets.size(), 4);  // rx, ry, rz, v
        uint idx = 0, idx_o = 0;
        for (const auto& v_pt : valid_targets)
          radar_data.row(idx++) =
              Vector4(v_pt[idx_.normalized_x], v_pt[idx_.normalized_y],
                      v_pt[idx_.normalized_z], v_pt[idx_.doppler]);
        
        // step: 2.3.2 估计ego_vel
        if (config_.use_ransac) {
          // step: ransac
          std::vector<uint> inlier_idx_best;
          std::vector<uint> outlier_idx_best;
          success = solve3DFullRansac(radar_data, v_r, sigma_v_r,
                                      inlier_idx_best, outlier_idx_best);
          // Insert inlier points to the cloud
          for (const auto& idx : inlier_idx_best)
            radar_scan_inlier->push_back(
                toRadarPointCloudType(valid_targets.at(idx), idx_));
          for (const auto& idx : outlier_idx_best)
            radar_scan_outlier->push_back(
                toRadarPointCloudType(valid_targets.at(idx), idx_));
        } else {
          // step: 所有点加入内点，求解
          for (const auto& item : valid_targets)
      ...
      ROS_INFO("To small valid_targets (< 2) in radar_scan (%ld)",
               radar_scan->size());
...
bool RadarEgoVelocityEstimator::solve3DFullRansac(
    const Matrix& radar_data, Vector3& v_r, Vector3& sigma_v_r,
  ...
  if (radar_data.rows() >= config_.N_ransac_points) {
    for (uint k = 0; k < ransac_iter_; ++k) {
      ...
      if (rtn) {
        ...
        // if too small number of inlier detected, the error is too high, so
        // regard outlier as inlier
        if (float(outlier_idx.size()) /
                (inlier_idx.size() + outlier_idx.size()) >
            0.05) {
          inlier_idx.insert(inlier_idx.end(), outlier_idx.begin(),
                            outlier_idx.end());
          outlier_idx.clear();
          // outlier_idx.swap(std::vector<uint>());
        }

        // ROS_INFO("Inlier number: %ld, Outlier number: %ld, outlier Ratio:
        // %f",
        //           inlier_idx.size(), outlier_idx.size(),
        //           float(outlier_idx.size())/(inlier_idx.size()+outlier_idx.size()));
...
/**
 * \brief // api: 所有观测估计ego_vel
 * 
 * \param radar_data 观测radar scan矩阵
 * \param v_r 估计的所有观测估计ego_vel
 * \param sigma_v_r 估计的所有观测估计ego_vel的方差
 * \param estimate_sigma true才会估计sigma_v_r
 * \return true 
 * \return false 
 */
bool RadarEgoVelocityEstimator::solve3DFull(const Matrix& radar_data,
  ...
  if (1)  // std::fabs(cond) < 1.0e3
  {
    ...
    if (estimate_sigma) {
      ...
      const Matrix C =
          (e.transpose() * e).x() * (HTH).inverse() / (H.rows() - 3);
      ...
      if (sigma_v_r.x() >= 0.0 && sigma_v_r.y() >= 0.0 && sigma_v_r.z() >= 0.) {
        ...
        sigma_v_r +=
            Vector3(config_.sigma_offset_radar_x, config_.sigma_offset_radar_y,
                    config_.sigma_offset_radar_z);
        if (sigma_v_r.x() < config_.max_sigma_x &&
            sigma_v_r.y() < config_.max_sigma_y &&
