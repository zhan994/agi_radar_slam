...
struct RadarEgoVelocityEstimatorConfig {
  ...
  float thresh_zero_velocity = 0.05;  // Below this is recognized as inlier
                                      // (m/s)
...
class RadarEgoVelocityEstimator {
 public:
  ...
  /**
   * @brief // api: 构造函数
   * RadarEgoVelocityEstimator constructor
   */
  RadarEgoVelocityEstimator() { setRansacIter(); }
   ...
   * Estimates the radar ego velocity based on a single radar scan
   ...
   */
  bool estimate(const sensor_msgs::PointCloud2& radar_scan_msg, Vector3& v_r,
                Vector3& sigma_v_r);
  bool estimate(const sensor_msgs::PointCloud2& radar_scan_msg, Vector3& v_r,
 ...
 private:
   ...
   * @returns true if estimation successful
   */
  bool solve3DFullRansac(const Matrix& radar_data, Vector3& v_r,
                         Vector3& sigma_v_r, std::vector<uint>& inlier_idx_best,
   ...
   * @returns true if estimation successful
   */
  bool solve3DFull(const Matrix& radar_data, Vector3& v_r, Vector3& sigma_v_r,
                   bool estimate_sigma = true);

  /**
   * @brief // api: 估计RANSAC迭代次数
   * Helper function which estiamtes the number of RANSAC iterations
   */
  void setRansacIter() {
    ransac_iter_ = uint((std::log(1.0 - config_.success_prob)) /
                        std::log(1.0 - std::pow(1.0 - config_.outlier_prob,
                                                config_.N_ransac_points)));
...
template <class ConfigContainingRadarEgoVelocityEstimatorConfig>
bool RadarEgoVelocityEstimator::configure(
    ConfigContainingRadarEgoVelocityEstimatorConfig& config) {
  ...
  config_.doppler_velocity_correction_factor =
      config.doppler_velocity_correction_factor;
  ...
  config_.use_cholesky_instead_of_bdcsvd =
      config.use_cholesky_instead_of_bdcsvd;
